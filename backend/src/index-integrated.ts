...
// Early reset-pin handler (always available; does not depend on auth module)
  ...
  try {
    ...
    if (idToken) {
      ...
      if (data.users?.[0]) {
        ...
        if (tokenPhone && tokenPhone === String(phone).replace(/\s/g, '')) verified = true;
      }
    }
    if (!verified && otp) {
    ...
    console.error('❌ [RESET PIN] Error:', err?.message || err);
    res.status(500).json({ message: 'Server error' });
  }
});
console.log('✅ [DEBUG] Early /api/auth/reset-pin handler registered');
