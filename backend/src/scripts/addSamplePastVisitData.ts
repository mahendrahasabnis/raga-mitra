...
import { sequelize } from '../config/database-integrated';
import {
  SharedUser,
  PlatformPrivilege
} from '../models-shared';
...
async function addSampleData() {
  try {
    ...
    const prescription1 = await PastPrescription.create({
      prescription_id: `PRX-${new Date().getFullYear()}-${Date.now().toString().slice(-8)}`,
    ...
    const receipt1 = await Receipt.create({
      receipt_id: `RCP-${new Date().getFullYear()}-${Date.now().toString().slice(-8)}`,
      appointment_id: pastVisit1.appointment_id,
      patient_id: patient.id,
      patient_name: patient.name,
    ...
    const receipt2 = await Receipt.create({
      receipt_id: `RCP-${new Date().getFullYear()}-${(Date.now() + 1).toString().slice(-8)}`,
      appointment_id: pastVisit1.appointment_id,
      patient_id: patient.id,
      patient_name: patient.name,
    ...
    const pastVisit2 = await PastVisit.create({
      appointment_id: `PV-${new Date().getFullYear()}-${(Date.now() + 2).toString().slice(-8)}`,
      patient_id: patient.id,
      patient_name: patient.name,
      patient_phone: patient.phone,
    ...
    const prescription2 = await PastPrescription.create({
      prescription_id: `PRX-${new Date().getFullYear()}-${(Date.now() + 3).toString().slice(-8)}`,
    ...
    const testResult1 = await PastTestResult.create({
      test_result_id: `TR-${new Date().getFullYear()}-${(Date.now() + 4).toString().slice(-8)}`,
      appointment_id: pastVisit2.appointment_id,
      patient_id: patient.id,
      patient_name: patient.name,
    ...
    const receipt3 = await Receipt.create({
      receipt_id: `RCP-${new Date().getFullYear()}-${(Date.now() + 5).toString().slice(-8)}`,
      appointment_id: pastVisit2.appointment_id,
      patient_id: patient.id,
      patient_name: patient.name,
      receipt_type: 'test',
