...
const DietOverview: React.FC<DietOverviewProps> = ({
  ...
  return (
    <div className="space-y-4 overflow-x-hidden">
      ...
      {nextSession && (
        <div className="card p-4 diet-overview-section">
          <div className="flex items-center gap-2 mb-3">
            <Calendar className="h-5 w-5 text-blue-200 diet-overview-section-icon" />
            <h3 className="text-md font-semibold">Next Meal Session</h3>
          </div>
          <div className="space-y-2">
            <p className="text-sm font-medium">{nextSession.session_name || "Meal Session"}</p>
            {nextSession.date && (
              <p className="text-xs text-gray-400">
                {new Date(nextSession.date).toLocaleDateString()}
              </p>
            )}
            {nextSession.items && nextSession.items.length > 0 && (
              <p className="text-xs text-gray-400">
                {nextSession.items.length} item{nextSession.items.length !== 1 ? "s" : ""}
              </p>
            )}
          </div>
        </div>
      ...
      {weekStats && (
        <div className="card p-4 diet-overview-section">
          <div className="flex items-center gap-2 mb-3">
            <Clock className="h-5 w-5 text-purple-200 diet-overview-section-icon" />
