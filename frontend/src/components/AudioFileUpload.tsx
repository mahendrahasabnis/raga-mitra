...
const AudioFileUpload: React.FC<AudioFileUploadProps> = ({ 
  ...
  const fetchExistingFiles = async () => {
    ...
    try {
      const response = await fetch('http://34.117.220.98/api/audio/files', {
        headers: {
  ...
  const deleteAudioFile = async (fileId: string) => {
    ...
    try {
      const response = await fetch(`http://34.117.220.98/api/audio/${fileId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const uploadFile = async (file: File): Promise<any> => {
    ...
    const response = await fetch('http://34.117.220.98/api/audio/upload', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${token}`
      },
