...
const RagaConfig: React.FC<RagaConfigProps> = ({ onBack }) => {
  ...
  const loadRagas = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://34.117.220.98/api/ragas', {
        headers: {
  ...
  const handleSave = async () => {
    try {
      const url = editingRaga 
        ? `http://34.117.220.98/api/ragas/${editingRaga._id}`
        : 'http://34.117.220.98/api/ragas';
  ...
  const handleDelete = async (ragaId: string) => {
    ...
    try {
      const response = await fetch(`http://34.117.220.98/api/ragas/${ragaId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      ...
      try {
        ...
        if (Array.isArray(importedRagas)) {
          ...
          try {
            ...
            const response = await fetch('http://34.117.220.98/api/ragas/batch-import', {
              method: 'POST',
              headers: {
                'Content-Type': 'application/json',
