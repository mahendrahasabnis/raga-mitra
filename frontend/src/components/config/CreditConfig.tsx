...
const CreditConfig: React.FC<CreditConfigProps> = ({ onBack }) => {
  ...
  const loadPackages = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://localhost:3001/api/credit-packages', {
        headers: {
  ...
  const handleSave = async () => {
    try {
      const url = editingPackage 
        ? `http://localhost:3001/api/credit-packages/${editingPackage._id}`
        : 'http://localhost:3001/api/credit-packages';
  ...
  const handleDelete = async (packageId: string) => {
    ...
    try {
      const response = await fetch(`http://localhost:3001/api/credit-packages/${packageId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (Array.isArray(importedPackages)) {
          const response = await fetch('http://localhost:3001/api/credit-packages/import', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('token')}`
