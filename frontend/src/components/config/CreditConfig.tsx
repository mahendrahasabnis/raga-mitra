...
const CreditConfig: React.FC<CreditConfigProps> = ({ onBack }) => {
  ...
  const loadPackages = async () => {
    try {
      setLoading(true);
      const response = await fetch('http://34.117.220.98/api/credit-packages', {
        headers: {
  ...
  const handleSave = async () => {
    try {
      const url = editingPackage 
        ? `http://34.117.220.98/api/credit-packages/${editingPackage._id}`
        : 'http://34.117.220.98/api/credit-packages';
  ...
  const handleDelete = async (packageId: string) => {
    ...
    try {
      const response = await fetch(`http://34.117.220.98/api/credit-packages/${packageId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (Array.isArray(importedPackages)) {
          const response = await fetch('http://34.117.220.98/api/credit-packages/import', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('token')}`
