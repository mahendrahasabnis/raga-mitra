...
const UserConfig: React.FC<UserConfigProps> = ({ onBack }) => {
  ...
  const handleSave = async () => {
    try {
      const url = editingUser 
        ? `http://localhost:3006/api/users/${editingUser._id}`
  ...
  const toggleUserStatus = async (userId: string, isActive: boolean) => {
    try {
      const response = await fetch(`http://localhost:3006/api/users/${userId}/status`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (Array.isArray(importedUsers)) {
          const response = await fetch('http://localhost:3006/api/users/import', {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${localStorage.getItem('token')}`
