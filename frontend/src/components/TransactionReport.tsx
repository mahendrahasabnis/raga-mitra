...
const TransactionReport: React.FC<TransactionReportProps> = ({ isOpen, onClose, isAdmin = false }) => {
  ...
  const loadUserTransactions = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`http://34.117.220.98/api/transactions/user/${user.phone}?page=${currentPage}&limit=10`, {
        headers: {
  ...
  const loadAllTransactions = async () => {
    setLoading(true);
    try {
      const url = new URL('http://34.117.220.98/api/transactions/admin/all');
  ...
  const loadSummary = async () => {
    try {
      const response = await fetch('http://34.117.220.98/api/transactions/admin/summary', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
  ...
  const loadStats = async () => {
    try {
      const response = await fetch('http://34.117.220.98/api/transactions/admin/stats', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('token')}`
        }
      });
